...
from PyQt5.QtWidgets import (QInputDialog,QFileDialog,QMessageBox,QWidget,QMainWindow,QDialog,QTabWidget,QSizePolicy,QTableWidget,QHBoxLayout,QGridLayout,QTableWidgetItem)
...
class Ui_MainWindow(QMainWindow):
    ...
    def setupUi(self, MainWindow):
        ...
        self.tabWidget.setGeometry(QtCore.QRect(35, 55,1600, 950))#, 1380, 900
        ...
        self.dummy_table = QtWidgets.QTableWidget()
        self.Add_Tool_Bar()
        self.Menu_bar()        
        #self.Add_Table()
    ...
    def Add_Table(self):
        ...
        if len(self.Loaded_Data_File_count)>=len(self.Table_List) and len(self.Loaded_Data_File_count)!=0:
            ...
            '''self.Table_model=QtCore.QAbstractTableModel()
            self.Table_List[0].setModel(self.Table_model)'''            
            ...
            self.Table_List[len(self.Table_List)-1].setGeometry(QtCore.QRect(0, 0, 350, 150))
            self.Table_List[len(self.Table_List)-1].setFocusPolicy(QtCore.Qt.NoFocus)
            self.Table_List[len(self.Table_List)-1].setObjectName("Table"+str(len(self.Table_List)-1))            
            self.Write_raw_data_into_data_table(self.Table_List[len(self.Table_List)-1],self.Loaded_Data_File_Raw_Data[self.Loaded_Data_File_count[len(self.Loaded_Data_File_count)-1]])
            ...
            '''self.Table_List[len(self.Table_List)-1].setGeometry(QtCore.QRect(0, 0, 350, 150))
            self.Table_List[len(self.Table_List)-1].setFocusPolicy(QtCore.Qt.NoFocus)
            self.Table_List[len(self.Table_List)-1].setColumnCount(1000)
            self.Table_List[len(self.Table_List)-1].setRowCount(1000)'''
            self.grid_layout.addWidget(self.Table_List[len(self.Table_List)-1],0,0)
    def Write_raw_data_into_data_table(self,Table,Data_Frame):
        '''Write the data into the data table created in Add_Table_Function'''
        #dtf=DF(self.Loaded_Data_Files[self.Loaded_Data_File_count[len(self.Loaded_Data_File_count)-1]]['Full_Rec_Summary'])
        ...
        self.Header_values=[]
        '''self.Header_values=list(Data_Frame.columns)
        for i in range(0,self.Row_count):
            for j in range(0,self.Column_Count):
                Table.setItem(i, j, QTableWidgetItem(Data_Frame.iat[i,j]))'''
    ...
    def Add_Tool_Bar(self):
        ...
        self.Data_Table_Icon = QtWidgets.QAction(self.Main_Window)
        self.Data_Table_Icon.setObjectName("actionExit")
        self.Data_Table_Icon.triggered.connect(self.Add_Table)
        self.Data_Table_Icon.setIcon(QtGui.QIcon("D:/2360/Scripting_data/Python_trail/GIT_HUB/Tool/STDF-READER-/Tool_GUI/Icons/Data_Table.png"))
        self.Tool_Bar.addAction(self.Data_Table_Icon)
