...
class READ_THE_STDF_FILE():
    ...
    def __init__(self,parent=None): #,Input_File
        ...
        self.Full_Rec_Summary={"File_Name":[],'Setuptime':[],'Strtuptime':[],'Stat_Num':[],'modcode':[],'retestcode':[],'protcode':[],'Burntime':[],'Lotid':[],'Parttype':[],'NodName':[],'TesterType':[],'Jobname':[],'Jobrev':[]
                                ,'Sublotid':[],'Opername':[],'TestTemp':[],'FlowId':[],'SetupId':[],'RomCode':[],'TestHeadnumber':[],'TestSiteNumber':[],'Part_Flag':[],'Num_Test':[],'HardBin':[],'Hbin_nam':[],'SoftBin':[],'Sbin_nam':[],'XCo_ord':[],'YCo_ord':[],'Testtime':[],'PartId':[],'Part_Dis_Txt':[],'PartFix':[],'IsNewPart':[],'Hbin_Num:Name':[],'Sbin_Num:Name':[]}
    ...
    def PTR(self,Starting_byte):
        ...
        if str(TestNumber)+";"+TestName not in self.TestNumbers and Starting_byte<self.End_Record_Byte:
            ...
            if LowLim!=None and TRSE!=None: LowLim=TestResult*(10**self.Scale_values[LLSE][2])
            if HighLim!=None and TRSE!=None: HighLim=TestResult*(10**self.Scale_values[HLSE][2])
        ...
        if CRFS!=None and CRFS!='': TestResult=round(TestResult,int(CRFS.split('.')[1].split('f')[0]))
        # OPT FLAG
        '''TRSE,Starting_byte=self.I1(Starting_byte,1) if OptFlag[7:8]!='1' else (None,Starting_byte)#;print('Test Result Scaling exponent :',TRSE)
        LLSE,Starting_byte=self.I1(Starting_byte,1) if (OptFlag[3:4]!='1' or OptFlag[1:2]!='1') else (None,Starting_byte)#;print('Low limit scaling exponent :',LLSE)
        HLSE,Starting_byte=self.I1(Starting_byte,1) if (OptFlag[2:3]!='1' or OptFlag[0:1]!='1') else (None,Starting_byte)#;print('High limit scaling exponent :',HLSE)
    ...
    def MRR(self,Starting_byte):
        ...
        for i in range(len(self.Full_Rec_Summary['HardBin'])):
            ...
            elif len(self.HBR_Rec_Summary['HbinNum'])==0:self.Full_Rec_Summary['Hbin_nam'].append(None)
            ...
            elif len(self.SBR_Rec_Summary['SbinNum'])==0:self.Full_Rec_Summary['Sbin_nam'].append(None)
            if len(self.HBR_Rec_Summary['HbinNum'])!=0:self.Full_Rec_Summary['Hbin_Num:Name'].append(self.Full_Rec_Summary['HardBin'][i]+":"+self.HBR_Rec_Summary['Hbin_nam'][self.HBR_Rec_Summary['HbinNum'].index(self.Full_Rec_Summary['HardBin'][i])])
            elif len(self.HBR_Rec_Summary['HbinNum'])==0:self.Full_Rec_Summary['Hbin_Num:Name'].append(None)
            if len(self.SBR_Rec_Summary['SbinNum'])!=0:self.Full_Rec_Summary['Sbin_Num:Name'].append(self.Full_Rec_Summary['SoftBin'][i]+":"+self.SBR_Rec_Summary['Sbin_nam'][self.SBR_Rec_Summary['SbinNum'].index(self.Full_Rec_Summary['SoftBin'][i])])
            elif len(self.SBR_Rec_Summary['SbinNum'])==0:self.Full_Rec_Summary['Sbin_Num:Name'].append(None)
